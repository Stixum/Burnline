...
- **There is no way to read subscription usage from Anthropic.** No API, no local file, no CLI — `/usage` renders to the terminal and exposes nothing. Confirmed 2026-08-11 against open Claude Code issues #21943 / #32796 / #44328; the Rate Limits API covers API organizations, not subscription plans. Calibration is the only bridge that exists. If that endpoint ever ships, replace `Calibration` and keep everything else.
