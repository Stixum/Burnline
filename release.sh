...
# Sparkle first: it is the deepest nesting in the bundle. Shared with build.sh
# rather than duplicated, so the two can never drift on signing order.
...
# --- 8. the appcast -------------------------------------------------------
# Generated here rather than by hand, for the same reason the cask sha is
# printed rather than stored: a feed maintained separately from the artefact it
# describes drifts from it, and a drifted feed is silently unusable — Sparkle
# rejects the signature and every installed copy simply stops updating.
echo "==> Signing the update and writing the appcast"
SIGN_UPDATE=$(find .build/artifacts -name "sign_update" -maxdepth 6 -not -path "*old_dsa*" | head -1)
if [ -z "${SIGN_UPDATE}" ]; then
  echo "!!! sign_update not found; run 'swift package resolve'." >&2
  exit 1
fi
# Reads the EdDSA private key from the login keychain. It is not on disk and
# must not be: losing it means no installed copy can ever update again.
if ! SIGNATURE_LINE=$("${SIGN_UPDATE}" "${DMG}" 2>&1); then
  echo "!!! sign_update failed. Is the Sparkle EdDSA key in the login keychain?" >&2
  echo "    ${SIGNATURE_LINE}" >&2
  exit 1
fi
ED_SIGNATURE=$(echo "${SIGNATURE_LINE}" | sed -n 's/.*sparkle:edSignature="\([^"]*\)".*/\1/p')
DMG_LENGTH=$(echo "${SIGNATURE_LINE}" | sed -n 's/.*length="\([^"]*\)".*/\1/p')
if [ -z "${ED_SIGNATURE}" ] || [ -z "${DMG_LENGTH}" ]; then
  echo "!!! Could not parse sign_update output: ${SIGNATURE_LINE}" >&2
  exit 1
fi

# `sparkle:version` is CFBundleVersion and is what Sparkle actually compares —
# it must increase on every release, independently of the human-facing version.
cat > docs/appcast.xml <<APPCAST
<?xml version="1.0" encoding="utf-8"?>
<!-- Generated by release.sh. Do not hand-edit: the signature below covers a
     specific DMG, and an edit that outruns the artefact silently stops every
     installed copy from updating. -->
<rss version="2.0" xmlns:sparkle="http://www.andymatuschak.org/xml-namespaces/sparkle">
  <channel>
    <title>Burnline</title>
    <link>https://stixum.github.io/Burnline/appcast.xml</link>
    <description>Updates for Burnline</description>
    ...
    <item>
      ...
      <sparkle:releaseNotesLink>https://github.com/Stixum/Burnline/releases/tag/v${VERSION}</sparkle:releaseNotesLink>
      <enclosure
...
echo
echo "    Then commit and push docs/appcast.xml — the release is not reachable"
echo "    by any installed copy until GitHub Pages serves the updated feed, and"
echo "    the enclosure URL 404s until the GitHub release itself exists."
echo
